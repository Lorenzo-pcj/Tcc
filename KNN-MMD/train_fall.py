...
def get_args():
    ...
    parser.add_argument("--data_path",type=str,default="./data") # Linha testando novo caminho
...
def dimension_reducation(data_loader,input_dim=(1,100,52),output_dim=128):
    ...
    print(f"Dimensão de dados de entrada (data_feature.shape): {data_feature.shape}") # Primeira checagem
    ...
    # Adicione este print para ter certeza
    print(f"Dimensões de dados de entrada carregadas: {data_feature.shape}")
    print(f"Dimensão de saída (output_dim): {output_dim}")

    reducer = umap.UMAP(n_components=output_dim)
    embedding = reducer.fit_transform(data_feature.numpy())
...
def main():
    ...
    while True:
        ...
        if acc >= best_acc or loss <= best_loss:
            torch.save(model.state_dict(), "./data/" + args.task + ".pth")
            torch.save(classifier.state_dict(), "./data/" + args.task + "_cls.pth")
        if acc >= best_acc:
            if acc == best_acc:
                same_epoch+=1
            else:
